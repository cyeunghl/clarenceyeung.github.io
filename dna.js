...
export function initDNA(container, options = {}) {
  const { reducedMotion = false, baseSpeed = 0.01 } = options;

  const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  const initialRect = container.getBoundingClientRect();
  const initialWidth = initialRect.width || container.clientWidth || 320;
  const initialHeight = initialRect.height || container.clientHeight || 320;
  ...
  function render() {
    ...
    if (!reduced) {
      currentSpeed = THREE.MathUtils.lerp(currentSpeed, targetSpeed, 0.08);
      helixGroup.rotation.y += currentSpeed;
      particles.rotation.y -= currentSpeed * 0.35;
    }
  ...
  function onScroll() {
    ...
    if (Math.abs(delta) > 1) {
      rotationDirection = delta > 0 ? 1 : -1;
      const intensity = THREE.MathUtils.clamp(Math.abs(delta) / 600, 0, 0.6);
      targetSpeed = baseSpeed * rotationDirection * (1 + intensity);
    } else {
      targetSpeed = baseSpeed * rotationDirection;
    }
  ...
  return {
    ...
    setReducedMotion(value) {
      reduced = value;
      targetSpeed = reduced ? 0 : baseSpeed * rotationDirection;
    },
    destroy() {
      isRunning = false;
      cancelAnimationFrame(animationFrameId);
