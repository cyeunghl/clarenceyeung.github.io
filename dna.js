...
export function initDNA(container, options = {}) {
  ...
  let rotationVelocity = baseSpeed;
  ...
  function render() {
    ...
    if (!reduced) {
      rotationVelocity = THREE.MathUtils.lerp(rotationVelocity, targetVelocity, 0.08);
      helixGroup.rotation.y += rotationVelocity;
      particles.rotation.y -= rotationVelocity * 0.45;
    }
  ...
  function onScroll() {
    ...
    if (Math.abs(delta) > 0.5) {
      targetVelocity = delta > 0 ? Math.abs(baseSpeed) : -Math.abs(baseSpeed);
    }
