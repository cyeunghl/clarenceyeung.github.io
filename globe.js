...
async function fetchActivities({ mockDataUrl, refreshIntervalDays, activityEndpoint }) {
  ...
  if (activityEndpoint) {
    try {
      ...
      if (response.ok) {
        ...
        console.warn('Strava API endpoint returned an error', response.status);
      }
    } catch (error) {
      console.warn('Unable to reach Strava API endpoint', error);
    }
  }
...
export async function initGlobe(container, options = {}) {
  const { mockDataUrl, refreshIntervalDays = 10, activityEndpoint } = options;
  const refreshIntervalMs = refreshIntervalDays * 24 * 60 * 60 * 1000;
