...
async function fetchActivities({ mockDataUrl, refreshIntervalDays, apiBase }) {
  const storageKey = apiBase
    ? `clarence-strava-cache::${apiBase}`
    : 'clarence-strava-cache';
  ...
  if (apiBase) {
    try {
      ...
      console.warn('Failed to load activities from Strava proxy', error);
    }
  }

  if (!payload) {
    if (mockDataUrl) {
      const response = await fetch(mockDataUrl, { headers });
      payload = await response.json();
    } else {
      payload = { activities: [] };
    }
...
export async function initGlobe(container, options = {}) {
  const { mockDataUrl, refreshIntervalDays = 10, apiBase } = options;
  const refreshIntervalMs = refreshIntervalDays * 24 * 60 * 60 * 1000;
